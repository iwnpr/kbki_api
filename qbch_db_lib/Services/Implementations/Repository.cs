...
namespace Qbch_db_lib.Services.Implementations
{
    ...
    public class Repository : IRepository
    {
        ...
        private readonly string[] _QBCHDB_ConnectionPool_old;
        private readonly string[] _CalcOfAmp_ConnectionPool_old;
        private readonly string[] _SearchSubjects_ConnectionPool_old;
        ...
        private readonly string? _schema_QbchDb_old;
        private readonly string? _schema_QbchSearchSubjects_old;
        private readonly string? _schema_QbchCalcOfAmp_old;
        ...
        public Repository(IConfiguration config, ILogger<Repository> logger, ICacheService cacheService)
        {
            ...
            // Пул подлючений к БД для версии 1.3
            _QBCHDB_ConnectionPool_old = config.GetSection("ConnectionPool_old:QbchDb").Get<string[]>();
            _CalcOfAmp_ConnectionPool_old = config.GetSection("ConnectionPool_old:QbchCalcOfAmp").Get<string[]>();
            _SearchSubjects_ConnectionPool_old = config.GetSection("ConnectionPool_old:QbchSearchSubjects").Get<string[]>();
        ...
        /// <param name="serviceName">Имя сервиса</param>
        /// <returns></returns>
        public async Task<bool> IsPermissionGranted(string? thumbprint, string? serviceName, CancellationToken? ct = null)
        {
            bool result = false;
            bool redisError = false;
            bool dbError = true;
            ...
            try
            {
                _cacheService.TryGetHashValue("permissions", thumbprint, serviceName, out var value);
            ...
            try
            {
                if (!redisError)
                    ...
                    {
                        // 2 Секунды
                        await _cacheService.AddHash("permissions", thumbprint, serviceName, result.ToString(), ct: ct);
                        await _cacheService.TrySetKeyExpiration("permissions", thumbprint, _permissionsLifeTime, ct: ct);
                    }).Wait(TimeSpan.FromMilliseconds(1500));
            }
            catch (Exception ex)
            {
        ...
        /// <param name="request">Запрос в формате xml</param>
        /// <returns>Список хидов</returns>
        public async Task<List<long>> GetSearchAllSubjects_old(string request)
        {
            var result = new List<long>();
            ...
            while (!ctsTotal.Token.IsCancellationRequested)
            {
                ...
                {
                    _logger.LogDebug(" | First db connection");
                    using var connection = GetDbConnection(0, _SearchSubjects_ConnectionPool_old);
                    try
                    {
                        await connection.OpenAsync(cts.Token);
                    ...
                    {
                        _logger.LogCritical(ex, "Ошибка процедуры SearchAllSubjects_old.");
                        await Task.Delay(_DBConnectDelayMs);
                    }
                    finally
                    {
                ...
                {
                    for (int i = 1; i < _SearchSubjects_ConnectionPool_old.Length; i++)
                    {
                        _logger.LogDebug(" | Connection string {i} from pool", i);
                        using var connection = GetDbConnection(i, _SearchSubjects_ConnectionPool_old);
        ...
        /// <param name="subjectIds">Хиды субъектов</param>
        /// <returns>XElement с данными о договорах</returns>
        public async Task<XElement?> GetCalculationOfAmp_old(List<long> subjectIds)
        {
            XElement? result = null;
            ...
            while (!cts.Token.IsCancellationRequested)
            {
                ...
                for (int i = 0; i < _CalcOfAmp_ConnectionPool_old.Length; i++)
                {
                    if (cts.Token.IsCancellationRequested)
                    {
                        _logger.LogDebug(" | cts CalculationOfAmp");
                        break;
                    }

                    _logger.LogDebug(" | Connection string {i} from pool", i);

                    using var connection = GetDbConnection(i, _CalcOfAmp_ConnectionPool_old);
                    try
                    {
                        await connection.OpenAsync();
                        using var cmd = new NpgsqlCommand(pgcmd, connection);
        ...
        /// <param name="thumbprint">Отпечаток</param>
        /// <returns>XElement ИНН ОГРН</returns>
        public async Task<XElement?> GetInnOgrnByThumbprint_old(string? thumbprint)
        {
            if (string.IsNullOrEmpty(thumbprint))
                return null;
            ...
            while (!cts.Token.IsCancellationRequested)
            {
                ...
                for (int i = 0; i < _QBCHDB_ConnectionPool_old.Length; i++)
                {
                    if (cts.Token.IsCancellationRequested)
                    {
                        _logger.LogDebug(" | cts GetInnOgrnByThumbprint");
                        break;
                    }
        ...
        /// <param name="psrn"></param>
        /// <returns></returns>
        public async Task<int?> GetAbonentKeyIdByPSRN_old(string? psrn)
        {
            int? result = null;
            ...
            while (!cts.Token.IsCancellationRequested)
            {
                ...
                for (int i = 0; i < _QBCHDB_ConnectionPool_old.Length; i++)
                {
                    ...
                    _logger.LogDebug(" | Connection string {i} from pool", i);
                    using var connection = GetDbConnection(i, _QBCHDB_ConnectionPool_old);
                    try
                    {
                        await connection.OpenAsync();
                        string pgcmd = $"select key_id from {_schema_QbchDb_old}.tr_abonents ta where ogrn = @psrn limit 1";
