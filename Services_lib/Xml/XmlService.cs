...
namespace Services_lib.Xml
{
    ...
    public class XmlService(IMemoryCache memoryCache, IConfiguration config, ILogger<XmlService> logger, ITicketService ticketService) : IXmlService
    {
            ...
            try
            {
                ...
                xDoc.Validate(schemaSet, (sender, e) =>
                {
                    ...
                    xsdError = new()
                    {
                        Error = error,
                        ErrorCode = 9,
                        Ticket_v2 = _ticketService.CreateResultv2(ResponseType.Error, "9", $"Запрос не соответствует схеме:\r\n{error}")
                    };
                });
                return xsdError;
            }
        ...
        private XmlSchemaSet GetXmlSchemaSet(string nameOfController, string version)
        {
            ...
            string[] requiredxsd = _config.GetSection($"XSD:{version}:{nameOfController}").Get<string[]>() ?? [];

            if(requiredxsd.Length == 0)
                _logger.LogError("Не найдены xsd схемы для проверки запроса.");
            ...
            try
            {
                ...
                xDoc.Validate(schemaSet, (sender, e) =>
                {
                    ...
                    xsdError = new()
                    {
                        Error = error,
                        ErrorCode = 9,
                        Ticket_v2 = _ticketService.CreateResultv2(ResponseType.Error, "9", $"Запрос не соответствует схеме: {error}")
                    };
                });
