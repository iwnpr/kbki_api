...
namespace Services_lib.QBCH
{
    public class QBCHService : IQBCHService
    {
        private readonly ICryptoAdapter _cryptoService;
        private readonly IXmlService _xmlService;
        private readonly ILogger<QBCHService> _logger;
        private readonly IDBAdapter _qbchDb;
        private readonly IRedisAdapter _redisCache;
        ...
        public QBCHService(ICryptoAdapter cryptoService, IXmlService xmlService, ILogger<QBCHService> logger, IDBAdapter qbchDb, IRedisAdapter redisCache, IConfiguration config)
        {
        ...
        /// <returns></returns>
        public async Task<QBCHTaskResult> AmpFromDB(QBCHProcessingTransaction processing)
        {
            ...
            foreach (var запрос in processing.ClentRequest.Request?.Запрос ?? [])
            {
                ...
                Обязательства? ampFromDlput = null;
                ...
                var dlputTask = _qbchDb.GetDlputData(xml, timeLeft);
                var BSIDTask = _qbchDb.GetSearchAllSubjects(xml, timeLeft);

                if (isAPMrequest)
                    await Task.WhenAll([BSIDTask, dlputTask]);

                // Получаем список SubjectId по титульной части xml
                List<long> BSIDArray = BSIDTask.Result;
                ...
                bool subjectFound = BSIDArray.Count != 0;

                // Заполняем данными по условию
                if (subjectFound)
                {
                    var SPTask = _qbchDb.GetSelfProhibition(BSIDArray, timeLeft);
                    var AMPTask = _qbchDb.GetCalculationOfAmp(BSIDArray, timeLeft);
                    ...
                    if (isAPMrequest)
                    {
                        ...
                        var xAmpDlput = dlputTask.Result;
                        ampFromDB = _xmlService.Deserialize<Обязательства>(xAmpDB);
                        ampFromDlput = _xmlService.Deserialize<Обязательства>(xAmpDlput);

                        if (ampFromDB is null && ampFromDlput is null)
                            noAmp = new ОбязательствНет();

                        if (ampFromDlput is not null)
                        {
                            ampFromDB ??= new();
                            ampFromDB.БКИ.AddRange(ampFromDlput.БКИ);
                        }
                    }
                }
                else
        ...
        /// <returns></returns>
        public async Task<QBCHTaskResult> AmpRequest(QBCHProcessingTransaction processing, HttpClient client, QBCHRequisite bureau)
        {
