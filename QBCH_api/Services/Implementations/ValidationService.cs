        ...
        public bool ValidateEncoding(byte[] message, [NotNullWhen(false)] out BaseResult? result)
        {
            ...
            catch (DecoderFallbackException ex)
            {
                ...
                result = new()
                {
                    ...
                    ErrorCode = 8,
                    Ticket_v2 = ticketService.CreateResultV2(ResponseType.Error, "8", "Неподдерживаемая кодировка, файл не в кодировке Utf-8")
                };
                return false;
            }
        ...
        public bool ValidateRequestDate(DateTime? requestDate, [NotNullWhen(false)] out BaseResult? result)
        {
            if (requestDate != DateTime.Today)
            {
                ...
                result = new()
                {
                    ...
                    ErrorCode = 23,
                    Ticket_v2 = ticketService.CreateResultV2(ResponseType.Error, "23", "Дата запроса указана некорректно")
                };
                return false;
            }
        ...
        public bool AbonentValidation(string? thumbprint, string? inn, string? ogrn, out AbonentValidatationResult result)
        {
            ...
            if (xInnOgrnDb is null || dbinn != inn || dbogrn != ogrn)
            {
                ...
                result = new(xInnOgrnDb)
                {
                    ...
                    ErrorCode = 10,
                    Ticket_v2 = ticketService.CreateResultV2(ResponseType.Error, "10", $"Реквизиты запроса не соответствуют абоненту: Абонент ИНН:{inn}, ожидаемый ИНН:{dbinn}. Абонент ОГРН:{ogrn}, ожидаемый ОГРН:{dbogrn}")
                };
                return false;
            }
        ...
        public bool AbonentValidation(string? thumbprint, string? ogrn, out AbonentValidatationResult result)
        {
            ...
            if (string.IsNullOrWhiteSpace(ogrn) || dbogrn != ogrn)
            {
                ...
                result = new(xInnOgrnDb)
                {
                    Error = "Реквизиты запроса не соответствуют абоненту",
                    ErrorCode = 10,
                    Ticket_v2 = ticketService.CreateResultV2(ResponseType.Error, "10", $"Реквизиты запроса не соответствуют абоненту: Абонент ОГРН:{ogrn}, ожидаемый ОГРН:{dbogrn}")
                };
                return false;
            }
        ...
        public bool ValidateAgreement(ЗапросСведенийОПлатежах? request, [NotNullWhen(false)] out BaseResult? result)
        {
            ...
            if (!AgreementDate.HasValue)
            {
                ...
                result = new()
                {
                    ...
                    Ticket_v2 = ticketService.CreateResultV2(ResponseType.Error, "13", "Отсутствует дата согласия.")
                };
            ...
            if (AgreementDate > DateTime.Today)
            {
                ...
                result = new()
                {
                    Error = error,
                    ErrorCode = 15,
                    Ticket_v2 = ticketService.CreateResultV2(ResponseType.Error, "15", $"Запрос содержит некорректные данные: {error}")
                };
                return false;
            }
                ...
                case КодыСрокаСогласия.I1:
                    if (DateTime.Today >= AgreementDate.Value.AddMonths(6).AddDays(1))
                    {
                        result = new()
                        {
                            ...
                            Ticket_v2 = ticketService.CreateResultV2(ResponseType.Error, "13", "Дата окончания действия согласия (дата выдачи + срок действия) меньше текущей даты")
                        };
                ...
                */
                case КодыСрокаСогласия.I3:
                    ...
                    if (request?.Запрос?.Согласие?.Договор is null)
                    {
                        result = new()
                        {
                            ...
                            Ticket_v2 = ticketService.CreateResultV2(ResponseType.Error, "15", "Элемент \"Договор\" обязателен т.к. значение атрибута \"СрокДействия\"=\"3\"")
                        };
                        return false;
                    }
                ...
                default:
                    result = new()
                    {
                        ...
                        Ticket_v2 = ticketService.CreateResultV2(ResponseType.Error, "13", $"Значение \"Срок действия\" блока \"Согласие\" {request?.Запрос?.Согласие?.СрокДействия}, не найдено в справочнике.")
                    };
            ...
            if (string.IsNullOrWhiteSpace(innAgreement))
            {
                result = new()
                {
                    ...
                    Ticket_v2 = ticketService.CreateResultV2(ResponseType.Error, "13", "В блоке \"Выдано\" отсутствуют реквизиты лица, которому было выдано согласие.")
                };
                return false;
            }
            if (string.IsNullOrWhiteSpace(ogrnAgreement))
            {
                result = new()
                {
                    ...
                    Ticket_v2 = ticketService.CreateResultV2(ResponseType.Error, "13", "Отсутствуют реквизиты лица, которому было выдано согласие.")
                };
                return false;
            }
            ...
            if (HasBasement)
            {
                ...
                if (ComapreINN)
                {
                    ...
                    result = new()
                    {
                        ErrorCode = 15,
                        Error = error,
                        Ticket_v2 = ticketService.CreateResultV2(ResponseType.Error, "15", error)
                    };
                    return false;
                }
            ...
            for (int i = 0; i < request?.Запрос?.Цель?.Count; i++)
            {
                if (!request?.Запрос.Согласие?.Цель?.Any(x => x.КодЦели == request.Запрос.Цель[i].КодЦели) ?? false)
                {
                    result = new()
                    {
                        ...
                        Ticket_v2 = ticketService.CreateResultV2(ResponseType.Error, "13", "Одна или несколько целей, указанных в блоке «Запрос» отсутствует.")
                    };
                ...
                */
                ...
                default:
                    result = new()
                    {
                        ...
                        Ticket_v2 = ticketService.CreateResultV2(ResponseType.Error, "13", $"Значение \"Срок действия\" блока \"Согласие\" {request?.Согласие?.СрокДействия}, не найдено в справочнике.")
                    };
        ...
        public bool IsUniqueRequestId(string requestId, string methodName, string ogrn, [NotNullWhen(false)] out BaseResult? validationResult)
        {
            ...
            if (!isUnuqie)
            {
                validationResult = new()
                {
                    ...
                    ErrorCode = 11,
                    Ticket_v2 = ticketService.CreateResultV2(ResponseType.Error, "11", $"Идентификатор запроса не уникален")
                };
                return false;
            }
        ...
        public bool ValidateQBCH(string? psrn, List<QBCHRequisite> QBCHList, [NotNullWhen(false)] out BaseResult? validationResult)
        {
            ...
            if (!IsValid)
            {
                validationResult = new()
                {
                    ...
                    ErrorCode = 11,
                    Ticket_v2 = ticketService.CreateResultV2(ResponseType.Error, "14", $"Взаимодействие с абонентом в режиме «одно окно» не предусмотрено договором")
                };
                return IsValid;
            }
