...
public static class DlrequestLogicalValidaton
{
            ...
            {
                {
                    switch (transaction.ClentRequest.Request.РежимЗапроса)
                    {
                        case СправочникРежимыЗапроса.Single:
                            ...
                            transaction.Dul999Validation(root);
                            transaction.Dul999IpValidation(root);
                            transaction.SubjectBrithDateValidation(root);
                            transaction.SubjectDocumentValidation(root);
                            transaction.LoanObligations(root);
                            return transaction;
                        case СправочникРежимыЗапроса.Package:
                            ...
                            requestCollection?.ForEach(r =>
                            {
                                if (!transaction.PackageValidationErrors.Any(x => x.Id == r.ПорядковыйНомер))
                                    transaction.Dul999IpValidation(r);
                                if (!transaction.PackageValidationErrors.Any(x => x.Id == r.ПорядковыйНомер))
                                    transaction.SubjectBrithDateValidation(r);
                                if (!transaction.PackageValidationErrors.Any(x => x.Id == r.ПорядковыйНомер))
                                    transaction.SubjectDocumentValidation(r);
                                if (!transaction.PackageValidationErrors.Any(x => x.Id == r.ПорядковыйНомер))
                                    transaction.LoanObligations(r);
                            });
                            return transaction;
                    }
                }
