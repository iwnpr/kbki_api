...
using Domain.QBCHModels.DTOs;
using Domain.QBCHModels.entities;
using Domain.QBCHRequisitsService;

namespace Domain.QBCHModels.aggregate;
...
public sealed class QBCHProcessingTransaction : AggregateRoot
{
    ...
    private QBCHProcessingTransaction(Guid id, string requestType, DateTime requestTime, ClentRequest clentRequest, Attachment attachment, List<QBCHRequisite> requisites) : base(id)
    {
    ...
    /// <param name="error"></param>
    public void SetPacakgeValidationError(int requestNumber, BaseError error)
    {
